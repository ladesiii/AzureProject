			...
			<form id="formCrearTarea" method="POST" action="{{ route('tareas.store') }}">
				...
				<div class="modal-body">
					...
					<div class="mb-3">
						...
						<textarea class="form-control custom-input" id="descripcionTarea" name="descripcion" rows="3" placeholder="Detalles de la tarea" required>{{ old('descripcion') }}</textarea>
					</div>
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						<select class="form-select custom-input" id="usuarioAsignado" name="id_usuario" required>
					...
					{{-- <div class="mb-3">
						...
						<select class="form-select custom-input" id="proyectoRelacionado" name="id_proyecto" required>
							<option value="" disabled {{ old('id_proyecto', 1) ? '' : 'selected' }}>Selecciona un proyecto</option>
							@isset($proyectos)
								@foreach($proyectos as $proyecto)
									<option value="{{ $proyecto->id_proyecto }}" @selected(old('id_proyecto', 1) == $proyecto->id_proyecto)>{{ $proyecto->nombre }}</option>
								@endforeach
							@endisset
						</select>
					</div> --}}
